...
function atualizarMapaReservas() {
    fetch('../backend/ler_reservas.php')
        .then(response => response.json())
        .then(data => {
            for (let i = 1; i <= 6; i++) {
...
function atualizarListaReservas() {
    fetch('../backend/buscar_proximas_reservas.php?t=' + new Date().getTime())
        .then(response => response.json())
        .then(data => {
...
function fazerCheckin() {
    ...
    fetch('../backend/fazer_checkin.php', { method: 'POST', body: formData })
    .then(response => response.json())
    .then(data => {
        alert(data.mensagem);
        if (data.sucesso) {
...
function enviarFeedback() {
    ...
    fetch('../backend/enviar_feedback.php', { method: 'POST', body: formData })
    .then(response => response.json())
    .then(data => {
        if (data.sucesso) {
